...
take_heals_sound = pygame.mixer.Sound("sound/healls.ogg")
...
screen_height = wind_height
screen = pygame.Surface((screen_width, screen_height))
###

# Create info string
info_string_width = wind_width
info_string_height = wind_height / 100 * 13
info_string = pygame.Surface((info_string_width, info_string_height))
...
hero_min_y = screen_height / 100 * 47
hero_max_y = screen_height / 100 * 85
hero_can_move = True
hero_x = screen_width / 100 * 40
hero_y = screen_height / 100 * 60
...
def spawn_loot():
    ...
    if loot == 1 and loot_spawned == False:
        ...
        bullets_loot.y = random.randint(screen_height / 100 * 50, screen_width / 100 * 90)
    if loot == 2 and loot_spawned == False:
        loot_spawned = True
        healls_loot.x = random.randint(screen_width / 100 * 10, screen_width / 100 * 90)
        healls_loot.y = random.randint(screen_height / 100 * 50, screen_height / 100 * 90)
...
def menu_buttons():
    ...
    if e.type == pygame.MOUSEBUTTONDOWN:
        if intersect(m_x, exit_button_position_x, m_y, exit_button_position_y, mouse_menu_width,
                     exit_button_width) == True:
            sys.exit()
        if intersect(m_x, play_button_position_x, m_y, play_button_position_y, mouse_menu_width,
                     play_button_width) == True:
            menu_start = False
            game_start = True
            game_play = True
            game_options()
...
screen_menu_hieght = wind_height
...
play_button_hieght = wind_height / 100 * 12.4

play_button_position_x = wind_width / 100 * 32.5
play_button_position_y = wind_height / 100 * 43.5
...
exit_button_hieght = wind_height / 100 * 12.4

exit_button_position_x = wind_width / 100 * 32.5
exit_button_position_y = wind_height / 100 * 70.4
...
def game():
    ...
    if game_start_first == True:
        ...
        wind_height += info_string_height
...
def drow():
    ...
    if win_or_lose_font == False:
        info_string.blit(hells_of_hero.render("Жизни: " + str(hero_hells), 1, red),
                         (screen_width / 100 * 69, screen_height / 100 * 0.70))
        info_string.blit(bullets_of_hero.render(u"ПУЛИ: " + str(bullets), 1, light_blue),
                         (screen_width / 100 * 1, screen_height / 100 * 0.83))
        info_string.blit(hells_of_enemy1.render(u"Враг 1: " + str(enemy1_hells) + "/10", 1, wight_blue),
                         (screen_width / 100 * 1, screen_height / 100 * 8))
        info_string.blit(hells_of_enemy1.render(u"Враг 2: " + str(enemy2_hells) + "/10", 1, wight_blue),
                         (screen_width / 100 * 68, screen_height / 100 * 8))
    if win_or_lose_font == True:
        info_string.blit(win_or_lose.render(winORlose, 1, red), (screen_width / 100 * 35, screen_height / 100 * 0.50))
    ###
